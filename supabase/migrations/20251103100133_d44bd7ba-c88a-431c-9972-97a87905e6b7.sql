-- Drop the restrictive policies that are blocking access
...
-- Create new permissive policies that work with both authenticated and anon roles
CREATE POLICY "Allow all access to lead generation"
ON public.lead_generation
FOR ALL
USING (true)
WITH CHECK (true);

CREATE POLICY "Allow all access to booked calls"
ON public.booked_call
FOR ALL
USING (true)
WITH CHECK (true);

CREATE POLICY "Allow all access to lead lavorati"
ON public.lead_lavorati
FOR ALL
USING (true)
WITH CHECK (true);

CREATE POLICY "Allow all access to booking clicks"
ON public.booking_clicks
FOR ALL
USING (true)
WITH CHECK (true);

CREATE POLICY "Allow all access to assignment history"
